...
class INA219 {
   public:
    ...
    // вернет ошибку или 0 если все хорошо
    //- 0   OK
    //- 1   DATA_TOO_LONG
    //- 2   RECEIVED_NACK_ON_ADDRESS
    //- 3   RECEIVED_NACK_ON_DATA
    //- 4   OTHER_ERROR
    //- 5   TIMEOUT
    uint8_t begin(int __attribute__((unused)) sda = 0, int __attribute__((unused)) scl = 0, const int address = 0x40) {
...
#endif
        uint8_t _test = testConnection();
        if (_test) return _test;  // Проверка присутствия
        calibrate();                          // расчёт калибровочного значения и инициализация
        return _test;                          // Вернуть true если все ок
    }
   ...
   private:
    ...
    uint8_t _iic_address = 0x00;  // Адрес на шине I2c
    ...
    uint8_t  testConnection(void) {
        Wire.beginTransmission(_iic_address);  // Начинаем передачу
        return Wire.endTransmission();  // Сразу заканчиваем, инвертируем результат
    }
